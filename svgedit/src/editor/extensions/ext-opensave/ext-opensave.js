...
export default {
	...
	async init (_S) {
		...
		// Ajouter une fonction pour ouvrir l'iframe
		// Fenêtre conversion SVG vers CSV
		const openCsv = () => {
			const iframe = document.createElement('iframe');
			iframe.src = './iframe/SvgToCsv.html'; 
			...
			iframe.style.backgroundColor = '#5499c7';
			document.body.appendChild(iframe);
			
			// Optionnel : Ajouter un bouton pour fermer l'iframe
			const closeBtn = document.createElement('button');
			document.body.appendChild(closeBtn);
			// Écouter le message du contenu de l'iframe pour fermer l'iframe
			window.addEventListener('message', function(event) {
				// Vérifier que le message est bien "closeIframe"
		...
		*/
		const importImage = (e) => {
			...
			if (file.type.includes('svg')) {
				...
				imgImport.value='' // RAZ fichier pour import Christophe Caron
			}
		}
		// create an input with type file to open the filesystem dialog
		const imgImport = document.createElement('input')
		imgImport.type = 'file'
		imgImport.accept = 'image/*'    // Selection image seulement Christophe Caron
		...
		const clickClear = async function () {
			...
			svgEditor.topPanel.updateTitle('nouveau.svg')
		}
		...
		*/
		...
		return {
			...
			callback () {
				...
				const saveAsButtonTemplate = '<se-menu-item id="tool_save_as" label="opensave.save_as_doc" src="saveImgS.svg"></se-menu-item>'
				svgCanvas.insertChildAtIndex($id('main_button'), saveAsButtonTemplate, 3)
				const importButtonTemplate = '<se-menu-item id="tool_import" label="tools.import_doc" shortcut="I" src="importImg.svg"></se-menu-item>'
				svgCanvas.insertChildAtIndex($id('main_button'), importButtonTemplate, 4)
				const iframeButtonTemplate = '<se-menu-item id="tool_open_csv" label="SVG=>CSV" shortcut="C" src="csv.svg"></se-menu-item>'
				svgCanvas.insertChildAtIndex($id('main_button'), iframeButtonTemplate, 5) // Ajouter à la fin du menu			
				// handler
				...
				$click($id('tool_open_csv'), openCsv)
			}
		}
	}
}
